...
def interactive_zone_picker(video_path):
    global clicked_points
    clicked_points = []
    ...
    if not ret or calibration_frame is None:
        print(f"Error: Could not grab calibration frame from video file: {video_path}")
        return None
...
def load_zone_coordinates(video_path):
    if os.path.exists(CONFIG_FILE):
        ...
        return interactive_zone_picker(video_path)


# --- VIDEO SOURCE SETUP ---
VIDEO_PATH = "/home/ju5ti5/zone/open-door.mp4"  # Update with your video file path

polygon = load_zone_coordinates(VIDEO_PATH)
if polygon is None:
...
while cap.isOpened():
    ...
    # Press 'c' to re-calibrate zone coordinates on the fly
        ...
        polygon = interactive_zone_picker(VIDEO_PATH)
        
        zone = sv.PolygonZone(polygon=polygon)
        zone_annotator = sv.PolygonZoneAnnotator(zone=zone, color=sv.Color.RED, thickness=2)
        
        # Re-open video capture after calibration
        cap = cv2.VideoCapture(VIDEO_PATH)
